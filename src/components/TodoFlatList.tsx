...
class TodoFlatList extends Component<IProps, IState> {
  state = {
    inputValue: '',
    todoList: [],
    tempId: '',
  ...
  handleUpdateTodo = (tempId: string) => {
    ...
    this.state.todoList.map((each: {id: string; inputValue: string}) => {
      if (each.id === tempId) {
        return (each.inputValue = this.state.inputValue);
      }
    });
        ...
        <View style={{flex: 1, width: '100%'}}>
          ...
          {this.state.todoList.length !== 0 ? (
            ...
            <ScrollView style={styles.editDeleteContainer}>
              <Text style={{textAlign:'center',fontSize: 24}}>Please Add Task</Text>
            </ScrollView>
          )}
